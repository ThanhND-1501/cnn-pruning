...
model = resnet50(pretrained=True, low_dim=1000)
...
for m in model.modules():
    if isinstance(m, nn.Conv2d):
        ...
        if layer_id % 2 == 0:
            ...
            median_norm = np.median(L1_norm)
            distance_to_median = np.abs(L1_norm - median_norm)
            num_keep = int(out_channels * (1 - prune_prob_stage))
            arg_min_dist = np.argsort(distance_to_median)[:num_keep]
            
            mask = torch.zeros(out_channels)
            mask[arg_min_dist.tolist()] = 1
...
for [m0, m1] in zip(model.modules(), newmodel.modules()):
    if isinstance(m0, nn.Conv2d):
        ...
        assert isinstance(m1, nn.BatchNorm2d), "There should not be a BN layer here."
        if conv_count % 2 == 1:
            mask = cfg_mask[layer_id_in_cfg-1]
            idx = np.squeeze(np.argwhere(np.asarray(mask.cpu().numpy())))
            if idx.size == 1:
...
torch.save({'cfg': cfg, 'state_dict': newmodel.state_dict()}, os.path.join(args.save, 'pruned_resnet50.pth.tar'))
